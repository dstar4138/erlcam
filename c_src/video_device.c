...
int open_device( const char* dev_name, int* fd ) {
    ...
    if( -1 == stat( dev_name, &st ) ) {
        DEBUG_PRINT("Cannot identify '%s': %d, %s", dev_name, errno, strerror(errno));
        return ERROR_LOCAL;
    }

    if( !S_ISCHR( st.st_mode ) ) {
        DEBUG_PRINT("%s is not a valid device", dev_name);
        return ERROR_LOCAL;
    }

    *fd = open(dev_name, O_RDWR | O_NONBLOCK, 0);
    if(-1 == *fd) {
        DEBUG_PRINT("Cannot open '%s': %d, %s", dev_name, errno, strerror(errno));
        if(EACCES == errno) {
            DEBUG_PRINT("Insufficient permissions on '%s': %d, %s", dev_name, errno,
                    strerror(errno));
        }
        return ERROR_LOCAL;
    }
...
int init_mmap( int fd ) {
    ...
    if(-1 == xioctl(fd, VIDIOC_REQBUFS, &req)) {
        if(EINVAL == errno) {
            DEBUG_PRINT("device does not support memory mapping");
            return ERROR_LOCAL;
        } else {
    ...
    if(req.count < 2) {
        DEBUG_PRINT("Insufficient buffer memory");
        return ERROR_LOCAL;
    }
...
int init_device( int fd, int width, int height, int format, int field ) {
    ...
    if( -1 == xioctl( fd, VIDIOC_QUERYCAP, &cap ) ) {
        if( EINVAL == errno ) {
            DEBUG_PRINT("not a valid V4L2 device");
            return ERROR_LOCAL;
        } else {
    ...
    if( !(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE) ) {
        DEBUG_PRINT("device is not a video capture device");
        return ERROR_LOCAL;
    }

    if( !(cap.capabilities & V4L2_CAP_STREAMING) ) {
        DEBUG_PRINT("device does not support streaming i/o");
        return ERROR_LOCAL;
    }
